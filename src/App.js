...
class App extends Component {
  state = {
    ...
    selected: imagesList[0].category,
    game: false,
    timer: 60,
    score: 0,
  }
  ...
  changeTheCategory = text => {
    this.setState({
      selected: text,
    })
  }

  changeTheStatusFunction = num => {
    const {randomNum} = this.state

    const checkingId = imagesList[randomNum].id
    ...
    if (num !== checkingId) {
      ...
      this.setState(each => ({
        score: each.score + 1,
        randomNum: Math.ceil(Math.random() * imagesList.length),
      }))
    }
  }
  ...
  render() {
    const {randomNum, thumbnailList, selected, game, timer, score} = this.state

    const randomImage = thumbnailList[randomNum].imageUrl

    const filterList = thumbnailList.filter(each => each.category === selected)
      ...
      <div className="overallContainer">
        <div className="headerSection">
          ...
          <div className="scoreAndTimerCard">
            <p className="para">
              {' '}
              Score:<span className="tick"> {score} </span>
            </p>
            <div className="timerSection">
              ...
              />
              <p className="para tick"> {timer} sec </p>
            </div>
          </div>
        </div>

        {game ? (
          <div className="playAgainCard">
            <img
              alt="trophy"
          ...
          <div className="bottomContainer">
            {' '}
            <div className="bottomSection para">
              <img
                alt={randomImage}
                src={randomImage}
            ...
            <ul className="tabs lists">
              {tabsList.map(each => (
                <TabItem
                  isActive={each.tabId === selected}
                  key={each.tabId}
                  passTheFunction={this.changeTheCategory}
                  passTheTab={each}
                />
              ))}
            </ul>
            <ul className="imagesCon lists">
              {filterList.map(each => (
                <li key={each.id}>
                  <Images
                    changeTheStatusFunction={this.changeTheStatusFunction}
                    send={each}
                  />
                </li>
              ))}
